import {CENTER, moveTypes, opponent, oppositeCorners}      from '../_lib/vars';
...
function blockForks(board, ch, random) {
  ...
  let allForksCorners = intersections.every((fork) => moveTypes.corners.includes(fork));
...
function playOppositeCorner(board, ch) {
  if(board[CENTER] === opponent[ch]) {
    let corners = moveTypes.corners.filter((corner) => board[corner] === ch);
